...
dataset = BrainDataset('./', transform=T.ToDense(max_nodes))
...
class Net(torch.nn.Module):
    def __init__(self, in_channels, out_channels, hidden_channels=128):
        ...
        num_cluster1 = 9   #This is a hyperparameter.
...
train_index = normal_index[2:3]  #This can extract a single graph for training.
...
run_number = 1
while run_number <= Times_Run:  #Generate multiple independent runs for soft consensus clustering.
    ...
    for epoch in range(1, TotalEpoch+1):  #Specify the number of epoch for training in each independent run.
