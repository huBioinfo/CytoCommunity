...
It remains poorly understood how different cell phenotypes organize and coordinate with each other to support tissue functions. To better understand the structure-function relationship of a tissue, the concept of tissue cellular neighborhoods (TCNs) has been proposed. There is a lack of computational tools for de novo identification of condition-specific TCNs by explicitly utilizing tissue image labels. Thus, given a set of tissue images associated with different conditions, it is often desirable to identify condition-specific TCNs with more biological and clinical relevance.

We developed the CytoCommunity algorithm for identifying TCNs. It can be applied in either an unsupervised or a supervised learning framework using single-cell spatial omics data. The direct usage of cell phenotypes as features to identify TCNs makes the algorithm applicable to spatial imaging data with relatively few features and facilitates the interpretation of TCN identity and functions as well. Additionally, CytoCommunity can not only infer TCNs for individual images but also identify condition-specific TCNs for a set of images by leveraging graph pooling and image labels, which effectively addresses the challenge of TCN alignment across images.
