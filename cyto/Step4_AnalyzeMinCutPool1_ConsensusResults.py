...
InputFolderName = "./MinCutPool_Brain_69KNNgraph_Input/"   #Modify 1/1.
...
#-----------------------------------------Generate plots-------------------------------------------------#
# Plot x/y map with "CytoCommunity" coloring. Note that consensus clustering result is generated by R with 1-indexing.
dict_color_CytoCommunity = {"1": "#5a3b1c", "2": "#939396", "3": "#2c663b", "4": "#d63189", \
    "5": "#54a9dd", "6": "#813188", "7": "Orange", "8": "#231f1f", "9": "#912b61"}
CytoCommunity_MajorityVoting_fig = sns.lmplot(x="x_coordinate", y="y_coordinate", data=target_graph_map, fit_reg=False, hue='PredictedLabel', legend=False, palette=dict_color_CytoCommunity, scatter_kws={"s": 6.0})

CytoCommunity_MajorityVoting_fig.set(xticks=[]) #remove ticks and also tick labels.
CytoCommunity_MajorityVoting_fig.set(yticks=[])
CytoCommunity_MajorityVoting_fig.set(xlabel=None) #remove axis label.
CytoCommunity_MajorityVoting_fig.set(ylabel=None)
CytoCommunity_MajorityVoting_fig.despine(left=True, bottom=True) #remove x(bottom) and y(left) axis.

CytoCommunity_MajorityVoting_fig.add_legend(label_order = ["5", "2", "4", "9", "7", "3", "6", "1", "8"])
for lh in CytoCommunity_MajorityVoting_fig._legend.legendHandles: 
    #lh.set_alpha(1)
    lh._sizes = [15]   # You can also use lh.set_sizes([15])
#plt.show()
# Save the figure.
CytoCommunity_fig_filename = "./CytoCommunity_MajorityVoting.pdf"
CytoCommunity_MajorityVoting_fig.savefig(CytoCommunity_fig_filename)
